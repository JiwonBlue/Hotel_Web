...
public interface BoardRepository extends JpaRepository<Board, Long> {
    // 해당 조건을 만족하는 게시물 검색
    List<Board> findByBoardCode(String board_code); // board_Code

    // 이 두 개의 매개변수를 사용하여 해당 조건을 만족하는 게시물을 검색
    List<Board> findByBoardCodeAndMemberId(String board_code, String member_id); // And, Or,...

    // member_id에 특정 문자열을 포함하는 모든 게시물을 조회하여 리스트로 반환
    List<Board> findByMemberIdContaining(String member_id); // XXXContaining()은 like연산자 역할
